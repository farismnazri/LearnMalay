...
## Open verification and notice review

Confirm contractual roles, receiver identity/contact records, Vercel Hobby DPA applicability and eligibility, provider subprocessors/support paths, access rosters, database security settings, deployment-retention policy, Vercel Observability and provider operational/audit-log retention, legal bases and rights language, whether legacy name-only scores exist and are attributable, external legal review and live enforcement of the operator-approved 90-day activity schedule, restore deletion propagation if future backups are enabled, and any service outside the reviewed repository and Vercel Integrations. Generated IDs must never be described as anonymous merely because they are not shown publicly.
