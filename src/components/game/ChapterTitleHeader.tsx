...
export default function ChapterTitleHeader({ chapterId, title, className }: ChapterTitleHeaderProps) {
  const chapterTitle = `CHAPTER ${chapterId}: ${normalizeChapterTitle(title)}`;
  const titleLines = splitTitleLines(chapterTitle);
  const chapterLengthClass =
    chapterTitle.length >= 52
  ...
  return (
    <header className={["crash-chapter-header", className].filter(Boolean).join(" ")}>
      <h1 className={["crash-chapter-title", chapterLengthClass].join(" ")} aria-label={chapterTitle}>
