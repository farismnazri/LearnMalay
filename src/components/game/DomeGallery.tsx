...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const step = (ts: number) => {
      ...
      // Keep spinning unless an image is currently focused/opened.
      if (!focusedElRef.current) {
        ...
        const el = sphereRef.current;
        if (el) {
          el.style.transform = `translateZ(calc(var(--radius) * -1)) rotateX(${nextX}deg) rotateY(${nextY}deg)`;
        }
      }
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }: any) => {
        ...
        if (last) {
          ...
          const [vMagX, vMagY] = velArr as [number, number];
          const [dirX, dirY] = dirArr as [number, number];
          let vx = vMagX * dirX;
          let vy = vMagY * dirY;
    ...
    <>
      ...
      >
        <main
          ...
          style={{
            touchAction: autoRotate ? "auto" : "none",
