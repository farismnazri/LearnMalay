...
export function addHighScore(gameId: GameId, entry: Omit<ScoreEntry, "id" | "dateISO"> & Partial<Pick<ScoreEntry, "id" | "dateISO">>) {
  ...
  db.prepare(
    `INSERT INTO highscores (id, game_id, name, accuracy, time_ms, date_iso, meta_json)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(full.id, gameId, full.name, full.accuracy, full.timeMs, full.dateISO, full.meta ? JSON.stringify(full.meta) : null);
